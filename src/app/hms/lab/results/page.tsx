...
import { getProcessingLabOrders, updateLabOrderStatus } from "@/app/actions/lab"
import { Button } from "@/components/ui/button"
import { Badge } from "@/components/ui/badge"
import { 
    Search, Filter, Clock, FlaskConical, ArrowRight, 
    CheckCircle2, Beaker, User, Calendar, Printer, Activity
} from "lucide-react"
...
export default function LabResultsProcessingPage() {
    ...
    async function loadOrders() {
        setLoading(true)
        const res = await getProcessingLabOrders()
        ...
        <div className="p-8 space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Samples collected and ready for diagnostic result entry</p>
                </div>
            ...
            <div className="grid grid-cols-1 gap-4">
                {loading ? (
                    <div className="h-64 flex flex-col items-center justify-center gap-4 text-slate-400">
                        ...
                        <p className="animate-pulse">Loading active processing queue...</p>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-900 border border-dashed border-slate-300 dark:border-slate-700 p-12 rounded-2xl text-center">
                        ...
                        <h3 className="text-xl font-bold text-slate-700 dark:text-white">No Processing Orders</h3>
                        <p className="text-slate-500">No samples waiting for result entry at this time.</p>
                    </div>
                ) : (
